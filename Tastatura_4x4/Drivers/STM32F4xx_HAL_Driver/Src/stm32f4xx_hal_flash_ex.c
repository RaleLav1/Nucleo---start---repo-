...
static HAL_StatusTypeDef FLASH_OB_UserConfig(uint8_t Iwdg, uint8_t Stop,
        uint8_t Stdby);
    ...
    if (status == HAL_OK)
    {
        ...
        *SectorError = 0xFFFFFFFFU;
        ...
        } else
        {
            /* Check the parameters */
            assert_param(
                    IS_FLASH_NBSECTORS(pEraseInit->NbSectors + pEraseInit->Sector));

            /* Erase by sector by sector to be done*/
            for (index = pEraseInit->Sector;
                    index < (pEraseInit->NbSectors + pEraseInit->Sector);
                    index++)
            {
...
HAL_StatusTypeDef HAL_FLASHEx_OB_SelectPCROP(void)
{
    ...
    /* Mask SPRMOD bit */
    optiontmp = (uint8_t) ((*(__IO uint8_t*) OPTCR_BYTE3_ADDRESS)
            & (uint8_t) 0x7F);

    /* Update Option Byte */
    *(__IO uint8_t*) OPTCR_BYTE3_ADDRESS = (uint8_t) (OB_PCROP_SELECTED
            | optiontmp);
...
HAL_StatusTypeDef HAL_FLASHEx_OB_DeSelectPCROP(void)
{
    ...
    /* Update Option Byte */
    *(__IO uint8_t*) OPTCR_BYTE3_ADDRESS = (uint8_t) (OB_PCROP_DESELECTED
            | optiontmp);
        ...
        uint8_t Stdby)
{
    ...
    if (status == HAL_OK)
    {
        ...
        optiontmp = (uint8_t) ((*(__IO uint8_t*) OPTCR_BYTE0_ADDRESS)
                & (uint8_t) 0x1F);

        /* Update User Option Byte */
        *(__IO uint8_t*) OPTCR_BYTE0_ADDRESS = Iwdg
                | (uint8_t) (Stdby | (uint8_t) (Stop | ((uint8_t) optiontmp)));
    }
...
static uint8_t FLASH_OB_GetRDP(void)
{
    ...
    } else if (*(__IO uint8_t*) (OPTCR_BYTE1_ADDRESS)
            == (uint8_t) OB_RDP_LEVEL_0)
    {
