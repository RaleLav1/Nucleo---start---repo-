...
#include "stm32f4xx.h"
    ...
    static const uint8_t aRCC_PLLSAIDIVRPrescTable[4] =
    {   2, 4, 8, 16};
...
#define __LL_RCC_CALC_PCLK2_FREQ(__HCLKFREQ__, __APB2PRESCALER__) ((__HCLKFREQ__) >> APBPrescTable[(__APB2PRESCALER__) >>  RCC_CFGR_PPRE2_Pos])
    ...
    {
        return (uint32_t) (READ_BIT(RCC->CR, RCC_CR_HSITRIM)
                >> RCC_CR_HSITRIM_Pos);
    }
  ...
  return (uint32_t)(READ_BIT(RCC->DCKCFGR, I2Sx) >> 16U | I2Sx);
    ...
    __STATIC_INLINE void LL_RCC_PLL_ConfigDomain_SYS(uint32_t Source,
            uint32_t PLLM, uint32_t PLLN, uint32_t PLLP_R)
    {
  ...
  MODIFY_REG(RCC->PLLCFGR, RCC_PLLCFGR_PLLR, PLLP_R);
    ...
    {
        return (uint32_t) (READ_BIT(RCC->SSCGR, RCC_SSCGR_INCSTEP)
    ...
    __STATIC_INLINE void LL_RCC_PLLI2S_ConfigDomain_I2S(uint32_t Source,
            uint32_t PLLM, uint32_t PLLN, uint32_t PLLR)
    {
        __IO uint32_t *pReg =
                (__IO uint32_t*) ((uint32_t) ((uint32_t) (&RCC->PLLCFGR)
                        + (Source & 0x80U)));
        MODIFY_REG(*pReg, RCC_PLLCFGR_PLLSRC, (Source & (~0x80U)));
#if defined(RCC_PLLI2SCFGR_PLLI2SM)
  MODIFY_REG(RCC->PLLI2SCFGR, RCC_PLLI2SCFGR_PLLI2SM, PLLM);
        ...
        MODIFY_REG(RCC->PLLI2SCFGR,
                ...
                PLLN << RCC_PLLI2SCFGR_PLLI2SN_Pos | PLLR);
    }
    ...
    {
        return (uint32_t) (READ_BIT(RCC->PLLI2SCFGR, RCC_PLLI2SCFGR_PLLI2SN)
