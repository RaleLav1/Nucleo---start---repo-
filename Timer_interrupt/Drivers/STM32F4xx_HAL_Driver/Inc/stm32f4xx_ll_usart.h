...
#define __LL_USART_DIV_SAMPLING16(__PERIPHCLK__, __BAUDRATE__)            (((__LL_USART_DIVMANT_SAMPLING16((__PERIPHCLK__), (__BAUDRATE__)) << 4) + \
            ...
            uint32_t Phase, uint32_t Polarity, uint32_t LBCPOutput)
    {
        MODIFY_REG(USARTx->CR2,
                USART_CR2_CPHA | USART_CR2_CPOL | USART_CR2_LBCL,
    ...
    {
        return (uint32_t) (READ_BIT(USARTx->CR3,
    ...
    __STATIC_INLINE void LL_USART_SetBaudRate(USART_TypeDef *USARTx,
            uint32_t PeriphClk, uint32_t OverSampling, uint32_t BaudRate)
    {
        if (OverSampling == LL_USART_OVERSAMPLING_8)
        {
            USARTx->BRR = (uint16_t) (__LL_USART_DIV_SAMPLING8(PeriphClk,
                    BaudRate));
        } else
        {
            USARTx->BRR = (uint16_t) (__LL_USART_DIV_SAMPLING16(PeriphClk,
                    BaudRate));
        }
    }
    ...
    __STATIC_INLINE uint32_t LL_USART_GetBaudRate(const USART_TypeDef *USARTx,
            uint32_t PeriphClk, uint32_t OverSampling)
    {
     ...
     * @retval State of bit (1 or 0).
     */
    __STATIC_INLINE uint32_t LL_USART_IsEnabledSmartcardNACK(
            const USART_TypeDef *USARTx)
    {
     ...
     */
    __STATIC_INLINE uint32_t LL_USART_GetSmartcardPrescaler(
            const USART_TypeDef *USARTx)
    {
        return (uint32_t) (READ_BIT(USARTx->GTPR, USART_GTPR_PSC));
    }
            ...
            uint32_t GuardTime)
    {
        MODIFY_REG(USARTx->GTPR, USART_GTPR_GT,
                GuardTime << USART_POSITION_GTPR_GT);
    }
     ...
     */
    __STATIC_INLINE uint32_t LL_USART_GetSmartcardGuardTime(
            const USART_TypeDef *USARTx)
    {
     ...
     *         @arg @ref LL_USART_LINBREAK_DETECT_11B
     */
    __STATIC_INLINE uint32_t LL_USART_GetLINBrkDetectionLen(
            const USART_TypeDef *USARTx)
    {
    ...
    __STATIC_INLINE void LL_USART_ConfigAsyncMode(USART_TypeDef *USARTx)
    {
        ...
        CLEAR_BIT(USARTx->CR3,
                (USART_CR3_SCEN | USART_CR3_IREN | USART_CR3_HDSEL));
    }
    ...
    __STATIC_INLINE void LL_USART_ConfigLINMode(USART_TypeDef *USARTx)
    {
        ...
        CLEAR_BIT(USARTx->CR3,
                (USART_CR3_IREN | USART_CR3_SCEN | USART_CR3_HDSEL));
    ...
    __STATIC_INLINE void LL_USART_ConfigSmartcardMode(USART_TypeDef *USARTx)
    {
        ...
        SET_BIT(USARTx->CR2,
                (USART_CR2_STOP_0 | USART_CR2_STOP_1 | USART_CR2_CLKEN));
    ...
    __STATIC_INLINE void LL_USART_ConfigIrdaMode(USART_TypeDef *USARTx)
    {
        ...
        CLEAR_BIT(USARTx->CR2,
                (USART_CR2_LINEN | USART_CR2_CLKEN | USART_CR2_STOP));
